...
class Collection(object):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        assert not (
            set(query.keys()) - set(groupby_attrs)
    ...
    def _assemble_cache_db(self):
        ...
        if cache_id_files:
            for file in cache_id_files:
                ...
                cache_origins_dict = {
                    k: cache_id[k] for k in self.origins_dict.keys() if k in cache_id
                }
    ...
    def _catalog_subset(self, variable, prefer_derived, refine_query):
        ...
        if len(catalog_subset_var):
            if is_derived and not prefer_derived:
                warnings.warn(f'found variable "{variable}" in catalog and derived_var registry')
                is_derived = False
        else:
            if not is_derived:
    ...
    def to_dataset_dict(
        self, variable, clobber=False, prefer_derived=False, refine_query={}, **kwargs
    ):
        """
        ...
        catalog_subset_var, is_derived = self._catalog_subset(
            variable, prefer_derived, refine_query
        )
    ...
    def _generate_dset(self, key, variable, is_derived, catalog_subset_var, key_info, **kwargs):
        ...
        assert list(dset.keys())[0] == key, f'mismatch in key:\nexpecting {key}\ngot: {dset.keys()}'
